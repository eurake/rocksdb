...
typedef std::shared_ptr<EvictAllVersionsFilterFactory>
    EvictAllVersionsFilterFactory_t;

// to ensure the lifetime of the listeners
std::vector<FlushBeginListener_t> all_flush_begin_listeners;
...
Status BlobDB::OpenAndLoad(const Options& options,
  ...
  *blob_db = nullptr;
  ...
  EvictAllVersionsFilterFactory_t comp_filt_factory =
      std::make_shared<EvictAllVersionsFilterFactory>();

  {
    MutexLock l(&listener_mutex);
    all_flush_begin_listeners.push_back(fblistener);
